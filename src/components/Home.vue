<template>
  <div class="row hfulli center">

    <div class="loading col" v-if="!user_data_loaded">

      <h4 class="animated pulse infinite text-white center align-self-center col">Loading ...</h4>

    </div>
    
    <div class="col" v-if="user_data_loaded">

      <div class="row hfulli">
...
export default {
  ...
  data() {
    const userDocRef = this.$stiller.firebase.firestore
      .collection("users")
      .doc(this.$stiller.firebase.auth.loggedUser.phone);
  ...
  methods: {
    likeClient() {
      this.$stiller.firebase.firestore
        .collection("users")
        .doc(this.$stiller.firebase.auth.loggedUser.phone)
        .update({
          type: "client"
